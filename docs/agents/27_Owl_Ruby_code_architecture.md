...
> Это правило соблюдается: в `lib/owl/cli/` нет ни одного обращения в доменный
> `Internal::*` (проверяется `grep -rn '::Internal::' lib/owl/cli/`, исключая
> `Cli::Internal::`). Прежний долг (~24 прямых обращения, доминировал
> `Steps::Internal::ActiveStepLock`) выпрямлен переводом каждого на
> additive-метод соответствующего `Api`-фасада. Исключение — `Owl::Cli::Internal::*`
> (собственный internal самого cli, напр. `UserFileReader`): это не cross-domain
> reach и под запрет не попадает.
