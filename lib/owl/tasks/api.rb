  ...
  module Tasks
    module Api
      ...
      def inspect(root:, task_id:)
        with_backend(root) { |backend| backend.inspect_task(task_id: task_id) }
      end

      def use(root:, task_id:)
        with_backend(root) { |backend| backend.use(task_id: task_id) }
      end

      def current(root:)
        with_backend(root, &:current)
      end

      def rebuild_index(root:)
        with_backend(root, &:rebuild_index)
      end
