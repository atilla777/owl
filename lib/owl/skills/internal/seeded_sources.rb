    ...
    module Internal
      module SeededSources # rubocop:disable Metrics/ModuleLength
        ...
        ORCHESTRATOR_BODY = <<~MD
          ...
          description: Drive an Owl task through its workflow end-to-end. Read state through the `owl` CLI, delegate each step to whichever skill the workflow YAML binds (typically the universal `owl-step-run`), stop on real human decisions.
          triggers: ["owl orchestrator", "continue owl task", "drive owl workflow", "next owl step"]
          ---

          ## Purpose

          Drive an Owl task from its current ready step to the workflow's terminal step using the `owl` CLI as the sole source of truth. The orchestrator is skill-binding-agnostic: it reads each ready step's `skill:` binding through `owl instructions` and delegates to whichever skill is declared. In the universal-step model that binding resolves to the `owl-step-run` skill; legacy seeded workflows may still bind per-step skills, and the orchestrator handles both without branching.
          ...
          4. Resolve the bound skill: `owl instructions TASK-ID --step-id STEP --json` returns the step invocation packaged with the matching `SKILL.md` path, slash-command path, and a one-paragraph summary. The `skill:` binding from the workflow YAML is authoritative — delegate to whatever it names (currently `owl-step-run` for migrated workflows, `owl-step-<id>` for legacy seeded ones). Use `owl step invocation TASK-ID STEP --json` when only the raw invocation block is needed.
          ...
          - In the universal-step model, every ready step's `skill:` binding resolves to the `owl-step-run` skill; that skill reads per-step `context` from `owl step show` and produces the declared artifact without hardcoded step-type knowledge. The orchestrator's job is to pick the step and trust the binding.
