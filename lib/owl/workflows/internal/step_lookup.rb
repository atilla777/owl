...
module Owl
  module Workflows
    module Internal
      module StepLookup
        STRING_FIELDS = %w[id title skill kind context default_variant session_type tier drift_policy].freeze
        ...
        def copy_string_field(step, normalized, field)
          value = step[field] || step[field.to_sym]
          normalized[field] = value.to_s if value
        end

        def copy_array_field(step, normalized, field)
          value = step[field] || step[field.to_sym] || []
