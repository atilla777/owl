...
- **Canonical next-task selection.** When there is no current pointer and no explicit `TASK-ID` (the "what should I work on next?" case), the canonical selector is `owl task available --json` (ranked by `priority` then age, each with an explainable `reason`, gated parents omitted) followed by `owl task claim --next` — this is exactly the Workflow step 1.4 ladder. Never pick the first entry of `owl task list`: that order is not a work-readiness ranking. If `owl task available` is empty, stop and report rather than guessing.
