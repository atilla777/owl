...
- **Step-execution serialization (current phase).** Task claims let sessions pick disjoint tasks, but `owl step start` still writes a single repo-wide `active_step` lock. If `owl step start` returns `active_step_locked` because another session is mid-step, wait and retry rather than `--force`; keep sessions on different tasks. (A later phase makes the step lock per-task.)
